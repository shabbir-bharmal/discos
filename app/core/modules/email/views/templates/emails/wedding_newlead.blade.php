...
Thank you for contacting me regarding your wedding on {{date("l jS \of F Y",strtotime($data->booking["date"]))}}.  I have pleasure in confirming my availability and providing you with a quotation for your event.<br><br>

To provide a wedding DJ between {{$data->booking["start_time_formatted"]}} and {{$data->booking["finish_time_formatted"]}} at {{$data->booking["venue_name"]}}, {{$data->booking["venue_postcode"]}} would be £{{$data->booking["total_cost"]}}.<br><br>
...
To place a booking and secure the date, simply <a href="{{ Config::get('app.url') . 'booking/' . $data->booking['email_token'] }}">click here</a>.<br><br>
