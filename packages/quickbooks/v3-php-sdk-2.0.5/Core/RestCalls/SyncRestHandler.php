...
//          var_dump($oauth->debugInfo['body_recv']);
//          echo "\n";
//          echo "ERROR MESSAGE: " . $oauth->debugInfo['body_recv'] . "\n"; // Useful info from Intuit
//          exit;

            if (isset($oauth->debugInfo['body_recv'])) {
                $msgObj = simplexml_load_string($oauth->debugInfo['body_recv']);
                Log::error("Error from QB: ".$msgObj->Fault->Error->Detail);

                throw new Exception($msgObj->Fault->Error->Message);
            }

            Log::error("Error from QB: ".$e->getMessage());
            throw $e;
        }
